...
(: Hacked a bit to get the keys from the database so that I could avoid having
   my keys and secrets in the source file that I put on github. :)
declare variable $config := doc("/etc/flickr-config.xml")/flickr:config;
declare variable $key    := string($config/flickr:key);
declare variable $secret := string($config/flickr:secret);
declare variable $token  := string($config/flickr:token);
